...
Dependencies point inward. Domain has no infrastructure dependency. Application describes persistence through narrow interfaces and infrastructure implements those boundaries.

```text
src/
├── Indtec.Labz.Catalog.Api
├── Indtec.Labz.Catalog.Application
├── Indtec.Labz.Catalog.Domain
└── Indtec.Labz.Catalog.Infrastructure

tests/
└── Indtec.Labz.Catalog.UnitTests
```
...
The application asks for the operations it actually needs through `IMusicRepository` and `ISetlistRepository`. A generic abstraction would expose capabilities the use cases do not require and would make persistence look simpler than it really is.
...
Publication time is an external dependency. Injecting the platform abstraction keeps the use case deterministic without inventing a custom `IClock` solely for architectural symmetry.
...
This lab deliberately does **not** start with MediatR, a generic repository, AutoMapper, a shared kernel, message brokers or a large hierarchy of base classes. Those tools can be useful, but none of them is a requirement for Clean Architecture, DDD or SOLID.
